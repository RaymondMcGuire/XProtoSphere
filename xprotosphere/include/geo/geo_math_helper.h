 ...
 * @LastEditTime: 2022-11-17 22:30:20
...
namespace KIRI::GEO::MATH_HELPER {

template <class T> inline constexpr T sqr(T x) { return x * x; }

template <class T> inline constexpr T cube(T x) { return x * x * x; }

template <class T> inline constexpr T pow3(T x) { return x * x * x; }

template <class T> inline constexpr T pow4(T x) { return sqr(sqr(x)); }

template <class T> inline constexpr T pow5(T x) { return pow4(x) * x; }

template <class T> inline constexpr T pow6(T x) { return pow3(sqr(x)); }

template <class T> inline constexpr T pow7(T x) { return pow6(x) * x; }

template <class T> inline constexpr T pow8(T x) { return sqr(pow4(x)); }

template <class T, class I> inline constexpr T pow(T x, I n) {
  T result = T(1);
  for (I i = 0; i < n; ++i) {
    result *= x;
  }
...
template <class T> inline constexpr T approx_sqrt(T x) {
  return x * approx_rsqrt(x);
}

template <class T> inline constexpr T norm2(T x1, T x2) {
  return std::sqrt(sqr(x1) + sqr(x2));
}

template <class T> inline constexpr T norm2(T x1, T x2, T x3) {
  return std::sqrt(sqr(x1) + sqr(x2) + sqr(x3));
}

template <class T> inline constexpr T norm3(T x1, T x2) {
  return std::pow(pow3(x1) + pow3(x2), T(1.0 / 3.0));
}

template <class T> inline constexpr T norm3(T x1, T x2, T x3) {
  return std::pow(pow3(x1) + pow3(x2) + pow3(x3), T(1.0 / 3.0));
}

template <class T> inline constexpr T norm4(T x1, T x2) {
  return std::pow(pow4(x1) + pow4(x2), T(1.0 / 4.0));
}

template <class T> inline constexpr T norm4(T x1, T x2, T x3) {
  return std::pow(pow4(x1) + pow4(x2) + pow4(x3), T(1.0 / 4.0));
}

template <class T> inline constexpr T norm5(T x1, T x2) {
  return std::pow(pow5(x1) + pow5(x2), T(1.0 / 5.0));
}

template <class T> inline constexpr T norm5(T x1, T x2, T x3) {
  return std::pow(pow5(x1) + pow5(x2) + pow5(x3), T(1.0 / 5.0));
}

template <class T> inline constexpr T norm6(T x1, T x2) {
  return std::pow(pow6(x1) + pow6(x2), T(1.0 / 6.0));
}

template <class T> inline constexpr T norm6(T x1, T x2, T x3) {
  return std::pow(pow6(x1) + pow6(x2) + pow6(x3), T(1.0 / 6.0));
}

template <class T> inline constexpr T norm7(T x1, T x2) {
  return std::pow(pow7(x1) + pow7(x2), T(1.0 / 7.0));
}

template <class T> inline constexpr T norm7(T x1, T x2, T x3) {
  return std::pow(pow7(x1) + pow7(x2) + pow7(x3), T(1.0 / 7.0));
}

template <class T> inline constexpr T norm8(T x1, T x2) {
  return std::pow(pow8(x1) + pow8(x2), T(1.0 / 8.0));
}

template <class T> inline constexpr T norm8(T x1, T x2, T x3) {
  return std::pow(pow8(x1) + pow8(x2) + pow8(x3), T(1.0 / 8.0));
}

template <class T> inline constexpr T norm_inf(T x1, T x2) {
  return std::max(std::abs(x1), std::abs(x2));
}

template <class T> inline constexpr T norm_inf(T x1, T x2, T x3) {
  return std::max(std::abs(x1), std::abs(x2), std::abs(x3));
}

template <class T> inline constexpr T min(T a1, T a2) {
  return std::min(a1, a2);
}

template <class T> inline constexpr T min(T a1, T a2, T a3) {
  return std::min(a1, std::min(a2, a3));
}

template <class T> inline constexpr T min(T a1, T a2, T a3, T a4) {
  return std::min(std::min(a1, a2), std::min(a3, a4));
}

template <class T> inline constexpr T min(T a1, T a2, T a3, T a4, T a5) {
  return min(std::min(a1, a2), std::min(a3, a4), a5);
}

template <class T> inline constexpr T min(T a1, T a2, T a3, T a4, T a5, T a6) {
...
template <class T> inline constexpr T max(T a1, T a2) {
  return std::max(a1, a2);
}

template <class T> inline constexpr T max(T a1, T a2, T a3) {
  return std::max(a1, std::max(a2, a3));
}

template <class T> inline constexpr T max(T a1, T a2, T a3, T a4) {
  return std::max(std::max(a1, a2), std::max(a3, a4));
}

template <class T> inline constexpr T max(T a1, T a2, T a3, T a4, T a5) {
  return max(std::max(a1, a2), std::max(a3, a4), a5);
}

template <class T> inline constexpr T max(T a1, T a2, T a3, T a4, T a5, T a6) {
...
template <class T> inline constexpr T clamp(T a, T lower, T upper) {
  if (a < lower) {
    return lower;
  } else if (a > upper) {
    return upper;
...
template <class T> inline constexpr T clamp01(T a) {
  if (a < T(0)) {
    return T(0);
  } else if (a > T(1.0)) {
    return T(1.0);
...
template <class T, class S> inline constexpr T clamp(T a, S lower, S upper) {
  if (a < T(lower)) {
    return T(lower);
  } else if (a > T(upper)) {
    return T(upper);
...
template <Int N, class T>
inline VectorX<N, T> clamp(const VectorX<N, T> &x, const VectorX<N, T> &a,
                           const VectorX<N, T> &b) {
  VectorX<N, T> result;
  for (Int d = 0; d < N; ++d) {
...
template <Int N, class T>
inline void clampInPlace(VectorX<N, T> &x, const VectorX<N, T> &a,
                         const VectorX<N, T> &b) {
  for (Int d = 0; d < N; ++d) {
...
template <class T> inline constexpr T smooth_step(T r) {
  if (r < 0) {
    return 0;
  } else if (r > T(1.0)) {
    return T(1.0);
...
// only makes sense with T=float or double
template <class T>
inline constexpr T smooth_step(T r, T r_lower, T r_upper, T value_lower,
                               T value_upper) {
...
// only makes sense with T=float or double
template <class T> inline constexpr T ramp(T r) {
  return smooth_step((r + T(1.0)) / T(2.0)) * T(2.0) - T(1.0);
}

template <class T> inline Int lround(T x) {
  if (x > 0) {
...
template <class T> inline double remainder(T x, T y) {
...
template <class T> inline void get_barycentric(T x, Int &i, T &f) {
  T s = std::floor(x);
  i = static_cast<Int>(s);
  f = (T)(x - s);
}
...
template <Int N, class T>
inline void get_barycentric(const VectorX<N, T> &x, VectorX<N, Int> &i,
                            VectorX<N, T> &f) {
  for (Int j = 0; j < N; ++j) {
    T s = std::floor(x[j]);
...
// {
//     return lerp(
//         trilerp(v0000, v1000, v0100, v1100, v0010, v1010, v0110, v1110, fx,
//         fy, fz), trilerp(v0001, v1001, v0101, v1101, v0011, v1011, v0111,
//         v1111, fx, fy, fz), ft);
// }
...
template <class T> inline constexpr T smin_exp(T a, T b, T k = T(32.0)) {
  T res = exp(-k * a) + exp(-k * b);
  return -log(res) / k;
}

// polynomial smooth min (k = 0.1);
template <class T> inline constexpr T smin_poly(T a, T b, T k = T(0.1)) {
  T h = clamp(T(0.5) + T(0.5) * (b - a) / k, T(0), T(1.0));
  return lerp(b, a, h) - k * h * (T(1.0) - h);
}

// power smooth min (k = 8);
template <class T> inline constexpr T smin_pow(T a, T b, T k = T(8.0)) {
  a = pow(a, k);
  b = pow(b, k);
  return pow((a * b) / (a + b), T(1.0) / k);
}

template <class T> T linear_kernel(T r, T h) {
  return r > h ? T(0) : T(1.0) - r / h;
}

template <class T> T quadratic_kernel(T r2, T h2) {
  return r2 > h2 ? T(0) : T(1.0) - r2 / h2;
}

template <class T> T sharp_kernel(T r2, T h) {
  if (r2 > h * h) {
    return 0;
  }
...
template <class T> T smooth_kernel(T r2, T h2) {
  T x2 = r2 / h2;
  return x2 > T(1.0) ? 0 : pow(T(1.0) - x2, T(3.0));
}

template <class T> inline constexpr T smooth_kernel_laplacian(T r2, T h2) {
  T x2 = T(sqrt(r2 / h2));
  return x2 > T(1.0) ? 0 : (T(1.0) - x2);
}

template <class T> inline constexpr T bilinear_kernel(T dx_, T dy_) {
  T dx = dx_ > 0 ? dx_ : -dx_;
  T dy = dy_ > 0 ? dy_ : -dy_;
...
template <class T> inline constexpr T trilinear_kernel(T dx_, T dy_, T dz_) {
  T dx = dx_ > 0 ? dx_ : -dx_;
  T dy = dy_ > 0 ? dy_ : -dy_;
  T dz = dz_ > 0 ? dz_ : -dz_;
...
template <class T> inline constexpr T spiky_kernel(T r, T h) {
  if (r > h) {
    return 0;
  }
  ...
  T val =
      T(315.0 / 64.0 / M_PI) * (T(1.0) - rh) * (T(1.0) - rh) * (T(1.0) - rh);

  return val;
}

template <class T> inline constexpr T quad_bspline_kernel(T f) {
  T x = f > 0 ? f : -f;
  T x2 = x * x;
...
template <class T> inline constexpr T quad_bspline_grad(T f) {
  T x = f > 0 ? f : -f;
  T x2 = x * x;
...
template <class T> inline constexpr T cubic_bspline_kernel(T f) {
  T x = f > 0 ? f : -f;
...
template <class T> inline constexpr T cubic_bspline_2d(T x, T y) {
  return cubic_bspline_kernel(x) * cubic_bspline_kernel(y);
}

template <class T> inline constexpr T cubic_bspline_3d(T x, T y, T z) {
  return cubic_bspline_kernel(x) * cubic_bspline_kernel(y) *
         cubic_bspline_kernel(z);
}

template <class T> inline constexpr T cubic_bspline_grad(T x) {
  T abs_x = x > 0 ? x : -x;
...
template <class T> inline void cycle_array(T *arr, Int size) {
  T t = arr[0];
...
// value < 0 is inside
template <class T> inline constexpr T fraction_inside(T left_val, T right_val) {
...
template <class T> T fraction_inside(T phi_bl, T phi_br, T phi_tl, T phi_tr) {
