 ...
 * @LastEditTime: 2022-11-24 15:53:09
...
inline __host__ __device__ float3 bilerp(const float3 &v00, const float3 &v10,
                                         const float3 &v01, const float3 &v11,
                                         float fx, float fy) {
  return lerp(lerp(v00, v10, fx), lerp(v01, v11, fx), fy);
}
...
trilerp(const float3 &v000, const float3 &v100, const float3 &v010,
        const float3 &v110, const float3 &v001, const float3 &v101,
        const float3 &v011, const float3 &v111, float fx, float fy, float fz) {
  return lerp(bilerp(v000, v100, v010, v110, fx, fy),
              bilerp(v001, v101, v011, v111, fx, fy), fz);
}
...
inline __host__ __device__ float3 interpolate_value_linear(
    const float3 point, const float3 *grid_data, const int3 grid_size) {
  int i, j, k;
  float fi, fj, fk;
  barycentric(point.x, i, fi, 0, grid_size.x);
  barycentric(point.y, j, fj, 0, grid_size.y);
