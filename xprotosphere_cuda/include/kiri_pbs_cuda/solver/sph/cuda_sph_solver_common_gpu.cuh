 ...
 * @LastEditTime: 2023-01-30 14:38:36
 * @FilePath:
...
_ComputeFieldColor_CUDA(float3 *col, const T *field, const float minVal,
                        const float maxVal, const size_t num,
                        ColormapType colorMapType) {
  const size_t i = __umul24(blockIdx.x, blockDim.x) + threadIdx.x;
  if (i >= num)
    return;
  float val = max(length(field[i]) - minVal, 0.f);
  float diff = abs(maxVal - minVal);
  val = min(val / diff, 1.f);
  col[i] = GetColor<float>(val, colorMapType).Data();
...
template <typename GradientFunc>
__device__ void
_ComputeFluidPressure(float3 *a, const size_t i, const float3 *pos,
                      const float *mass, const float *density,
                      const float *pressure, const size_t cellStart,
                      const size_t cellEnd, GradientFunc nablaW) {
  size_t j = cellStart;
  while (j < cellEnd) {
    *a -= mass[j] *
